...
function promptRegister()
{
    ...
    if ( !promptOptions.description )
        $( '.description' ).remove();
    else if ( promptOptions.useHtmlDescription )
        $( '.description' ).innerHTML = promptOptions.description;
    else
        $( '.description' ).textContent = promptOptions.description;

    // set button label
    if ( promptOptions.buttonLabels?.ok )
        $( '.btn-ok' ).textContent = promptOptions.buttonLabels.ok;

    if ( promptOptions.buttonLabels?.cancel )
        $( '.btn-cancel' ).textContent = promptOptions.buttonLabels.cancel;

    if ( !promptOptions.description )
        $( '.label' ).setAttribute( 'class', 'p-btm' );
    ...
    $( '.btn-cancel' ).addEventListener( 'click', promptCancel );

    // create input/select/counter/keybind
    const dataContainerElement = $( '.elements' );
    ...
    if ( promptOptions.button )
    {
        ...
        button.addEventListener( 'click', () =>
            // This is a library providing the option to add a custom function to developers
...
function promptSubmit()
{
    ...
    switch ( promptOptions.type )
    {
        ...
        case 'multiInput':
            data = Array.from( document.querySelectorAll( '.item' ) ).map(
...
function promptCreateCounter( parentElement )
{
    ...
    dataElement.onwheel = ( event ) =>
    {
        const value = Number.parseInt( dataElement.value );
...
/*
    ...
    @param {object} timer {
        time: First delay in milliseconds
